...
const useForm = (initialValues = {}, onFieldsChange, onValuesChange, scrollToFirstError, onFinish) => {
  ...
  const getFieldInstance = React.useCallback(name => {
  ...
  const getFieldsValue = React.useCallback(nameList => {
    const formData = getFormFields();
  ...
  const getFieldError = React.useCallback(name => {
    return errorsRef.current[name] || [];
  }, []);
  const getFieldWarning = React.useCallback(name => {
  ...
  const validateField = React.useCallback(async name => {
  ...
  const setFieldValue = React.useCallback((name, value, errors, reset = undefined, touch) => {
  ...
  const setFieldsValue = React.useCallback((values, reset) => {
    Object.entries(values).forEach(([name, value]) => setFieldValue(name, value, undefined, reset));
  }, [setFieldValue]);
  const setFields = React.useCallback(fields => {
    fields.forEach(({
      name,
      value,
      errors
  ...
  const setFieldInstance = React.useCallback((fieldName, fieldRef) => {
    fieldInstancesRef.current[fieldName] = fieldRef;
  }, []);
  const isFieldTouched = React.useCallback(name => {
    return touchedFieldsRef.current.has(name);
  }, []);
  const isFieldsTouched = React.useCallback((nameList, allFieldsTouched = false) => {
    if (!nameList) {
  ...
  const isFieldValidating = React.useCallback(name => {
    return !!name;
  }, []);
  const registerField = React.useCallback((name, rules = [], remove = false) => {
    if (remove) {
  ...
  const validateFields = React.useCallback(async nameList => {
  ...
  const resetFields = React.useCallback((nameList, showError = true) => {
    const formData = getFormFields();
    if (nameList?.length) {
  ...
  const subscribeToField = React.useCallback((name, callback) => {
  ...
  const setScrollToFirstError = React.useCallback(value => {
    _scrollToFirstError.current = value;
  }, []);
  const setOnFieldsChange = React.useCallback(cb => {
    formHandlersRef.current.onFieldsChange = cb;
  }, []);
  const setOnValuesChange = React.useCallback(cb => {
    formHandlersRef.current.onValuesChange = cb;
  }, []);
  const setOnFinish = React.useCallback(cb => {
    formHandlersRef.current.onFinish = cb;
  }, []);
  const changeStep = React.useCallback(step => {
...
const FormItem$1 = /*#__PURE__*/React.memo(({
...
const FormItemChildComponent = /*#__PURE__*/React.memo(({
  child,
  name,
  error,
  initialValue,
