    ...
    return (
        <div style={{ width: 700, margin: "0 auto" }}>
            ...
            <Form form={form} size="large" layout="vertical" onFinish={(e) => console.log(e)}>
                {!isHide ?
                    <>
                        <Form.Item normalize={(value: string) =>
                            value.trimLeft().replace(/\s{2,}/, ' ')} name="input" label="Input" rules={[{ required: true }]}>
                ...
                <button onClick={() => console.log(form.getFieldInstance('checkbox'))}>Submit</button>
                <button type="button" onClick={() => form.resetFields()}>Reset</button>
                <button type="button" onClick={hideForm}>hide</button>
            </Form>
            {/* 
            <h1>Ant Form</h1>
            <AntForm form={antForm} size="large" layout="vertical" onFinish={(e) => console.log(e)}>
                <AntForm.Item normalize={(value: string) =>
                    value.trimLeft().replace(/\s{2,}/, ' ')} name="input" label="Input" rules={[{ required: true }]}>
                    <AntInput allowClear suffix={<>sf</>} prefix={<>pf</>} addonAfter={<>aa</>} addonBefore={<>ab</>} />
                </AntForm.Item>
                ...
                <AntForm.Item name="checkbox" layout="horizontal" label="Checkbox" rules={[{ required: true }]}>
                    <AntCheckbox name="Check1">Check 1</AntCheckbox>
                    <AntCheckbox name="Check2">Check 2</AntCheckbox>
                </AntForm.Item>

                <AntForm.Item name="radio" layout="horizontal" label="Radio" rules={[{ required: true }]}>
                    <AntRadio value="8" title="8">8</AntRadio>
                    <AntRadio value="false" title="false">false</AntRadio>
                </AntForm.Item>

                <AntForm.Item name="datepicker" label="Date Picker" rules={[{ required: true }]}>
                    <AntDatePicker value={new Date()} onChange={() => { }} />
                </AntForm.Item>

                <button onClick={() => console.log(antForm.getFieldInstance('checkbox'))}>Submit</button>
