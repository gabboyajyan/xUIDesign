...
const DatePicker = ({
  ...
  const [selectedDatePlaceholder, setSelectedDatePlaceholder] = useState<string>();
  ...
  const [viewMode, setViewMode] = useState<'day' | 'month' | 'year'>(picker === 'month' ? 'month' : picker === 'year' ? 'year' : 'day');

  const localeMonths = locale?.shortMonths || Array.from({ length: 12 }, (_, i) =>
    new Date(0, i).toLocaleString(locale?.locale || 'default', { month: 'short' })
  );
  ...
  const days = [
    ...Array.from({ length: firstDay }, (_, i) => ({ day: prevMonthDays - firstDay + i + 1, current: false, month: prevMonth, year: prevMonthYear })),
    ...Array.from({ length: totalDays }, (_, i) => ({ day: i + 1, current: true, month: currentMonth, year: currentYear })),
    ...Array.from({ length: nextDaysCount }, (_, i) => ({ day: i + 1, current: false, month: nextMonth, year: nextMonthYear }))
  ];

  return (
    <div
      ref={containerRef}
      className={cc([`${prefixCls}-container`,
      {
        noStyle,
        [`${prefixCls}-${size}`]: size
      }
      ...
      <div className={`${prefixCls}-input-wrapper`}>
        <button
          ...
          <span
            className={cc([`${prefixCls}-selected-date`,
            {
          ...
          <span className={`${prefixCls}-icon`}>
            {allowClear && selectedDate ? (
              <span
                ...
                {typeof allowClear === 'object' && allowClear.clearIcon ? allowClear.clearIcon : <ClearIcon />}
              </span>
          ...
          <div className={`${prefixCls}-dropdown`}>
            <div className={`${prefixCls}-header`}>
              <div className={`${prefixCls}-nav-buttons`}>
                ...
                <button onClick={() => setCurrentMonth(m => m === 0 ? (setCurrentYear(y => y - 1), MONTH_LENGTH) : m - 1)}>&lsaquo;</button>
              </div>
                ...
                {days.map(({ day, current, month, year }, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={cc([`${prefixCls}-day`, { [`${prefixCls}-selected`]: isSelected, [`${prefixCls}-other-month`]: !current }])}
                      onClick={() => handleSelect(day, month, year)}
                      disabled={disabledDate?.(new Date(year, month, day), { from: undefined, to: undefined })}
                    >
                      {day}
                    </button>
                  );
            ...
            {showToday && (
              ...
              >
                <button
                  className={`${prefixCls}-select`}
                  disabled={disabledDate?.(new Date(DateNow.getDate(), DateNow.getMonth(), DateNow.getFullYear()), { from: undefined, to: undefined })}
                  onClick={() => { handleSelect(DateNow.getDate(), DateNow.getMonth(), DateNow.getFullYear()); }}>
                  {locale?.today || 'Today'}
                </button>
              </div>
            )}
